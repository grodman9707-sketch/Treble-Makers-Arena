...
`/api/commentary`, `/api/wdl-standings`, and `/api/lazy-standings`. Optional AI
commentary (single or dual announcers) uses Groq + Deepgram TTS. There is no
external DB, no test suite, and no lint config; state lives in a single JSON
file. Standard commands are in `README.md` / `package.json`.
